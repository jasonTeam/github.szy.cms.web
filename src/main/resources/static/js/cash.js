...
require(['config', 'jquery', 'storm', 'common', 'WdatePicker'], function (CONFIG, $, S) {
  ...
  var $form = $('[data-form="search"]'),//查询用户提现记录表单
      ...
      $formReset = $form.find(':reset'),//重置按钮
  	  ...
  	  $exprotAnchor = $('.exprot_anchor'),// 导出按钮
  	  $newexprotAnchor = $('.newexprot_anchor');// 导出按钮
...
//	  $formBtn.click();//查询
	  ...
	  $exprotAnchor.click(exprotAnchor);
	  $newexprotAnchor.click(newexprotAnchor);
  ...
  //主播详情导出
  function exprotAnchor() {
	    S.confirm('导出会消耗服务器性能,你确定要导出吗?', function () {
	  	  var data = {};
		  data[$form.data('param')] = JSON.stringify($form.formData());
		  location.href = CONFIG.addr.anchorMessageExprot+'?' + $.param(data);
	    });
  }
  
//主播详情导出
  function newexprotAnchor() {
	    S.confirm('导出会消耗服务器性能,你确定要导出吗?', function () {
	  	  var data = {};
		  data[$form.data('param')] = JSON.stringify($form.formData());
		  location.href = CONFIG.addr.newAnchorMessageExprot+'?' + $.param(data);
	    });
  }
