...
+function ($) {
    var defaultOptions = {
        ...
        floatStyle: {
            ...
            'webkitTransform': 'rotate(0.1deg)',
            'mozTransform': 'rotate(0.1deg)',
            'msTransform': 'rotate(0.1deg)',
    ...
    $.fn.DDSort = function (options,callback) {
    	
    	var thisId = null,thisSortNum = null;
    	var changeId = null,changeSortNum = null;
        ...
        return this.each(function () {
            ...
            that.on('mousedown.DDSort touchstart.DDSort', settings.target, function (e) {
                ...
                $doc.on('mousemove.DDSort touchmove.DDSort', function (e) {
                	...
                	thisSortNum = $this.attr('data-sortNum');
                	
                	console.log(thisId)
                	console.log(thisSortNum)
                    
                    // 向上排序
                    if (prev.length && top < prev.offset().top + prev.outerHeight() / 2) {
                    	console.log('prev');
                    	
                    	changeId = prev.attr('data-id');
                    	changeSortNum = prev.attr('data-sortNum');
                    	console.log(changeId);
                    	console.log(changeSortNum);
                    	
                        clone.after(prev);
                        settings.changeNum(thisId,thisSortNum,changeId,changeSortNum);
                        
                        $this.attr('data-sortNum',changeSortNum);
                        prev.attr('data-sortNum',thisSortNum);
                        
                        // 向下排序
                    } else if (next.length && top + thisOuterHeight > next.offset().top + next.outerHeight() / 2) {
                    	console.log('next');
                    	
                    	changeId = next.attr('data-id');
                    	changeSortNum = next.attr('data-sortNum');
                    	console.log(changeId);
                    	console.log(changeSortNum);
                        ...
                        settings.changeNum(thisId,thisSortNum,changeId,changeSortNum);
                        $this.attr('data-sortNum',changeSortNum);
                        next.attr('data-sortNum',thisSortNum);
                    }
                    ...
                    if (!hasClone) {
                        ...
                        clone.before($this.removeAttr('style').attr('style','cursor:move')).remove();
