...
//    	  }
	 ...
	 * @returns {*|{}}
	 */
    autoLayer: function (docHeight,docScroll) {//弹框自适应高度
//		var docHeight = document.body.clientHeight;//获取页面可视                             区域高度
//		var docScroll = document.body.scrollTop;//获取页面滚动条高度
		var modalHeight = $('.modal-content').height(); 
		
		var poorNum = (parseInt(docHeight) - parseInt(modalHeight)) / 2;
		var topNum = poorNum + parseInt(docScroll);
	...
	getQueryString: function (name) {
		var src = $(window.parent.document).find("#mainFrame").attr("src");
	...
	*/
    ...
    outsert: function (name) {
      ...
      array.forEach(function (value) {
        var $this = $('[data-insert="' + name + '"]') , type = $this.is('input') ? ($this.is('[type="radio"]') || $this.is('[type="checkbox"]') ? 'checked' : 'val') : 'html';
        obj[value] = $this[type]();
      });
	 ...
	 * @returns {*}
	 */
    template: function (json, template, json_) {
      var temp = template
          , index = 0
          , oldIndex = 0
          , i = 0
          , code = '\'use strict\';\nvar r = \'\', _this = this;\n'
          , match, length, tem, number = 0, size = 0;
        Storm.debug && (code += 'debugger;\n');
        match = temp.match(/{{(.| )+?}}/g);
        length = match.length;
        if ("undefined" !== typeof(json_)) {
    		number = json_.number;
            size = json_.size;
        }
        for (; i < length; i++) {
