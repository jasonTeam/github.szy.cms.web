...
//		$('#wp_addActive').show();
		...
		$('#active_showimg').attr('src','http://opq77mx3q.bkt.clouddn.com/170515-156-156.png');
		$('#share_showimg').attr('src','http://opq77mx3q.bkt.clouddn.com/20170710-share_logo.png');
	}
	
	$("#active_upimg").change(function() {//直播间logo图片上传
		var data = {};
	...
	$("#share_upimg").change(function() {//直播间分享logo图片上传
		var data = {};
		$("#wp_shareupload").ajaxSubmit({
			type: "post", // 提交方式 get/post
			url: CONFIG.addr.fileupload, // 需要提交的 url
			data: data,
			success: function(data) {
	...
	function activeModify() {
		...
		$this.postBtn(CONFIG.addr.findLiveH5, $this.data(), function(data) {
			...
			$('#active_showimg').attr('src',activeIco);
			$('#share_showimg').attr('src',sharePicUrl);
			
			$('.isEnable_class').val(isEnables);
			$('.isShare_class').val(isShare);
	...
	function activeModifyTrue(e) {
		...
		$modifyActive.validate(function() {
			$modifyActive.setForm(function() {
				...
				$('.isEnable_class option').removeAttr('selected');
				$('.isShare_class option').removeAttr('selected');
